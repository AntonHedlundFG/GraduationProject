...
## Keywords
**Engine:** Unreal Engine 5, C++, [AngelScript by HazeLight](https://angelscript.hazelight.se/), [Epic Online Services](https://dev.epicgames.com/en-US/services).
\
...
## Introduction
This is the report that covers my graduation project at Futuregames in Stockholm, undertaken from March 18 to May 24. This project offered an opportunity to explore and practice with the capabilities of Unreal Engine while integrating it with [Hazelight's AngelScript](https://angelscript.hazelight.se/). 
...
Online session management was handled using Epic Online Services for Unreal Engine. We utilized a modified version of Unreal Engine 5.3.0, which included support for [Hazelight's AngelScript](https://angelscript.hazelight.se/), to build the game.
...
- **Procedural Generation:** To implement procedural generation for major systems in the game such as loot and the game grid. 
- **Seeded Random:** A seeded approach to the randomness in the game ensuring deterministic outcomes.
- **Command-pattern:** To use the command-pattern for game actions, allowing players to undo all the actions that they have made.
- **Functionally Identical Units:** Ensuring that the units function identically for both players and AI. The AI should interact with the game exactly as the player does, with access to no more or less information than the player has.
...
In the initial planning of the project, we laid out a flexible plan with milestones for each week. The plan served as a guide rather than a fixed schedule, allowing us to adapt as needed throughout the project.
...
In the initial startup of the project, the team planned to use Discord for online communication, GitHub for version control and Jira for task management.\
We also planned to have a stand-up meeting, daily, at 10 AM. We wouldn't have weekly reviews of the project, but we would make sure to have a weekly build from week 2 and onwards. Review meetings for the whole team were to be planned and scheduled for when there were a need.

### Mentorship and Guidance
The school provided us with a budget of 1 hour or mentoring per week, per students. Since we were five student collaborating, we had a combined budget of 5 hours a week. We decided as a group to reach out to two mentors and divided our time between them.
#### Marc Zaku
Marc Zaku was the teacher in our Design Pattern course. We reached out to him as we were planning a very system heavy project that required a robust core architecture. Knowing his extensive knowledge in this subject, we believed he would be an excellent mentor for us.
#### Ian Bradley
Ian Bradley had been available to the class during our introductory C++ course.
...
### Team Goals Achieved
Overall, the team's goals we mostly fulfilled. We managed base implementations for everything we set out to do. Some aspects of the plans weren't fully realized though, while some other features which did receive more development time, instead ended up more advanced and feature rich than we had planned.
...
- **Seeded Random:** The system is functional and in use; however, since the seed is not currently exposed to the user, it might as well be unseeded.
...
- **Team Collaboration:** Collaboration within the team has been great. I also feel more confident working in a code base where I haven't had a hand in every part of it, and don't know the ins and outs of everything.
...
### Game AI Development  
I took responsibility for the AI in the game, and I ended up as the sole programmer and implementer of the system for the game. Given that we were developing a game with a wide variety of actions and items, I aimed to develop and implement a robust AI system capable of handling all this complexity. After researching for a few days, I ended up deciding in on implementing an Utility AI system to control the game's enemies. 

Utility AI, or Utility-based AI, is a system designed to assign utility scores to various potential actions that the AI can take. The scores represent the usefulness of each action in the current context which the AI is in. The AI then selects the action with the highest utility score. My system also recursively evaluates all the units possible actions in all potential sequences to plan out each round of actions before beginning their execution.
...
I am very happy to have collaborated with the other four team members on this graduation project. We aimed to develop a turn-based tactical rogue-lite game with quite a few complex features. Reflecting on the journey, I believe that it has been a significant success for both me and the team. Despite the challenged we encountered, we achieved most of our primary goals. The knowledge and skills that I have gained, especially in system design and AI development, will be invaluable for future projects.
